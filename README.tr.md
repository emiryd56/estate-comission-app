...
| Test Süreçleri | Jest (Backend birim ve servis testleri), `vue-tsc` / `tsc` tip denetimleri |
...
Tarayıcınızda [http://localhost:3000](http://localhost:3000) adresine gidin. Başlatma kodu esnasında terminalde konsola yazdırılan varsayılan admin bilgileri (genellikle admin@company.com / admin123 şeklindedir) ile ilk girişinizi yapın.
...
| `SEED_ADMIN_EMAIL` | hayır | `admin@company.com` | `npm run seed` oluşturduğunda kurulacak ilk varsayılan admin hesabıdır. |
...
| --- | --- | --- |
| admin (Yönetici) | `admin@company.com` | `admin123` |
| agent (Danışman) | `alex@company.com` | `agent123` |
| agent (Danışman) | `priya@company.com` | `agent123` |
| agent (Danışman) | `james@company.com` | `agent123` |

> **Terminoloji notu:** Uygulamada ve veritabanında `UserRole.AGENT` (değer: `'agent'`) olarak tanımlanan rol, Türkçe belgelendirmede "Danışman" olarak adlandırılır; her iki isim de aynı rolü ifade eder.
...
```bash
# Backend birim ve servis testleri (Jest; komisyon, aşama geçişleri ve servis orkestrasyonu dahil 44 senaryo):
npm test
...
| GET | `/auth/me` | tüm giriş yapanlar | Sistemin kullanıcı değerini sağladığı onay alanı (`{ userId, name, email, role }`). |
...
- **Denetim izi (Audit trail)** — `StageHistoryEntry` her aşama geçişinde `changedBy` alanı ile kim tarafından tetiklendiğini kaydeder; işlem detay sayfasındaki dikey zaman çizelgesi bu bilgiyi "{tarih} · {isim} tarafından" satırıyla doğrudan gösterir.
